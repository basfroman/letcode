...
# from the other by rearranging the digits (or trivially, if the numbers are equal). For example, 54275 and 45572 are
# digit anagrams, but 321 and 782 are not (since they don't contain the same digits). 220 and 22 are also not considered
# as digit anagrams, since they don't even have the same number of digits.
#
# Example
#
...
def solution(a):
    ...
    for item in a:
        string = ''.join(sorted(str(item)))

        table[string] += 1
        counter += table[string] - 1
